 ...
 * Fish
 */
var fishX = Math.floor(((Math.random()*900)-30)+1);
var fishY = Math.floor(((Math.random()*300)-30)+1);

var fish = new Vector(fishX, fishY);
...
Player.prototype.draw = function(ct) {
    ...
    // draw background pic
    if(bgReady) {
        ct.drawImage(bgImage, 0, 0, 900, 400);
    }
  ...
  if (Key.isDown(Key.UP, Key.W)) {
    ...
    ct.lineTo(4, y+20);
    ct.lineTo(0, y);
    ct.fillStyle = 'hsla(0,100%,50%,0.2)';
    ct.fill();
  }
  ...
  // Draw fish
   ...
   // Is the ship touching the fish?
   var fish_left = fish.x,
    fish_top = fish.y,
    fish_right = fish.x + 150,
    fish_bottom = fish.y + 100;
    ...
    if (
      ...
      && y <= fish_bottom
       ...
       fish.x = Math.floor(((Math.random()*900)-150));
       fish.y = Math.floor(((Math.random()*300)-100));
   }
   ct.drawImage(fishImage, fish.x, fish.y, 150, 100);
